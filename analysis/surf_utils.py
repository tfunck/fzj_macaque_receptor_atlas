...
def project_to_surface(
    ...
    for receptor_volume in receptor_volumes:
        ...
        base = os.path.basename(receptor_volume).replace('.nii.gz','')

        profile_fn = f"{output_dir}/{base}.func.gii"
        profile_smooth_fn = f"{output_dir}/{base}_smoothed.func.gii"

        if surf_smooth == 0 :
            profile_list.append(profile_fn)
        else :
            profile_list.append(profile_smooth_fn)

        n_step = int(bound1 - bound0)

        print(wm_surf_filename, gm_surf_filename)
        if not os.path.exists(profile_list[-1]) or clobber :
            ...
            wm_coords, triangles = mesh_utils.load_mesh_ext(wm_surf_filename, correct_offset=True)
            gm_coords, _ = mesh_utils.load_mesh_ext(gm_surf_filename, correct_offset=True)

            profiles = np.zeros([wm_coords.shape[0], 1+int(n_step)])

            d_vtr = ( wm_coords - gm_coords ).astype(np.float64)

            for i, d in enumerate(np.linspace(bound0/n, bound1/n, n_step).astype(np.float64)):
                print('d', d)    
                coords = gm_coords + d * d_vtr

                # write coordinates to surf.gii 
                #coords_fn = f'{output_dir}/coords_{d}.surf.gii'
                #nib.save(nib.gifti.GiftiImage(darrays=[
                #        nib.gifti.GiftiDataArray(coords.astype(np.float32), intent="NIFTI_INTENT_POINTSET"), # coordinates
                #        nib.gifti.GiftiDataArray(triangles, intent="NIFTI_INTENT_TRIANGLE") # triangles
                #    ]), coords_fn)

                # use wb_command to project the volume to the surface
                #surf_values_fn = f"{output_dir}/{base}_{d}.func.gii"
                #cmd = f'wb_command -volume-to-surface-mapping {receptor_volume} {coords_fn} {surf_values_fn} -ribbon-constrained {wm_surf_filename} {gm_surf_filename}'
                #print(cmd)
                #subprocess.run(cmd, shell=True, executable="/bin/bash")
                #surf_receptor_vtr = nib.load(surf_values_fn).darrays[0].data

                #interp_vol, _ = mesh_utils.mesh_to_volume(
                #    coords,
                ...
                #    steps,
                #)
                #assert np.sum(interp_vol) > 0, f"Interpolated volume is empty for {d}"  
                #nvol_out[ interp_vol > 0 ] =  1 + d

                surf_receptor_vtr = mesh_utils.volume_filename_to_mesh(coords, receptor_volume, zscore=False)
            ...
            if mask is not None:
                profiles[mask==0] = np.nan

            # save profiles as func.gii with nibabel
            write_gifti(profiles, profile_fn)


            #print(f'{output_dir}/{base}_nvol.nii.gz'); 
            #nib.Nifti1Image(nvol_out, receptor_img.affine).to_filename(f'{output_dir}/{base}_nvol.nii.gz')
            if surf_smooth > 0:
                # Use wb_command to smooth the surface
                cmd=f'wb_command -metric-smoothing {wm_surf_filename} {profile_fn} {surf_smooth} {profile_smooth_fn} -fix-zeros'
                print(cmd)
                subprocess.run(cmd, shell=True, executable="/bin/bash")
                assert os.path.exists(profile_smooth_fn), f"Could not find smoothed profile {profile_smooth_fn}"
                profile_fn = profile_smooth_fn
            
            plot_receptor_surf(
                [profile_fn], wm_surf_filename, output_dir, label=base, cmap='RdBu_r', threshold=[0,100]
            )
...
def plot_receptor_surf(
    ...
    if not os.path.exists(filename) or clobber :
        ...
        print(receptor_all.shape)

        receptor = np.mean( receptor_all,axis=(0,2))
        print('Receptor', receptor.shape)
        ...
        print(receptor[~np.isnan(receptor)])
