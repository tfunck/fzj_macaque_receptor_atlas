...
from volumetric.surf_utils import plot_receptor_surf
import volumetric.cca_analysis as cca 
import matplotlib.pyplot as plt
import seaborn as sns
from sklearn.linear_model import LinearRegression
import pandas as pd
from volumetric.surf_utils import write_gifti
...
def cell_type_analysis(receptor_surfaces, cortex_mask_filename, surface_filename, sphere_filename, output_dir, clobber:bool=False):
    ...
    cortex_mask = nib.load(cortex_mask_filename).darrays[0].data.astype(bool)
    ...
    if not os.path.exists(df_csv) or clobber:
        ...
        Y0, Y0_mask = cca.preprocess(receptor_surfaces, kind='receptor')
        ...
        gaba_cols = [ 'GABA' in col for col in X.columns ]
        glu_cols = [ 'GLU' in col for col in X.columns ]    

        cell_gaba = X.iloc[:, gaba_cols]
        cell_glut = X.iloc[:, glu_cols]
        ...
        n = 4
        cell_gaba_comp = pca(cell_gaba, mask, 'cell_gaba', surface_filename, output_dir, n=n)
        cell_glut_comp = pca(cell_glut, mask, 'cell_glut', surface_filename, output_dir, n=n)
        ...
        df = receptor_cell_regr(
            cell_comp, receptor, output_dir, surface_filename, mask, avg_labels, labels, clobber=clobber
            )
        df.to_csv(df_csv)
    else:
